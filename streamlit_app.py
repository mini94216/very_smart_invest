...
def calculate_metrics(df):
    ...
    # 如果 Excel 中有週轉率，直接使用；否則計算
    if inventory_turnover_col is not None:
        df['存貨週轉率（次）'] = df.iloc[:, inventory_turnover_col].astype(float)
    else:
        # 如果沒有直接的數據，才進行計算
        inventory_col = get_column_by_keyword(df, ['存貨'])
        if revenue_col is not None and inventory_col is not None:
            df['存貨週轉率（次）'] = [
                get_numeric_value(df.iloc[i, revenue_col]) / max(get_numeric_value(df.iloc[i, inventory_col]), 0.1)
                for i in range(len(df))
            ]
    
    if ar_turnover_col is not None:
        df['應收帳款週轉次數'] = df.iloc[:, ar_turnover_col].astype(float)
    else:
        ar_col = get_column_by_keyword(df, ['應收帳款及票據', '應收帳款'])
        if revenue_col is not None and ar_col is not None:
            df['應收帳款週轉次數'] = [
                get_numeric_value(df.iloc[i, revenue_col]) / max(get_numeric_value(df.iloc[i, ar_col]), 0.1)
                for i in range(len(df))
            ]
    
    if ap_turnover_col is not None:
        df['應付帳款週轉次數'] = df.iloc[:, ap_turnover_col].astype(float)
    else:
        ap_col = get_column_by_keyword(df, ['應付帳款及票據'])
        if revenue_col is not None and ap_col is not None:
            df['應付帳款週轉次數'] = [
                get_numeric_value(df.iloc[i, revenue_col]) / max(get_numeric_value(df.iloc[i, ap_col]), 0.1)
                for i in range(len(df))
            ]
    
    # 週轉天數計算：直接使用 90 ÷ 週轉率
    if '存貨週轉率（次）' in df.columns:
        df['存貨週轉天數'] = [
            90 / max(get_numeric_value(df.iloc[i]['存貨週轉率（次）']), 0.1)
            for i in range(len(df))
        ]
    
    if '應收帳款週轉次數' in df.columns:
        df['應收帳款週轉天數'] = [
            90 / max(get_numeric_value(df.iloc[i]['應收帳款週轉次數']), 0.1)
            for i in range(len(df))
        ]
    
    if '應付帳款週轉次數' in df.columns:
        df['應付帳款週轉天數'] = [
            90 / max(get_numeric_value(df.iloc[i]['應付帳款週轉次數']), 0.1)
            for i in range(len(df))
        ]
...
with st.sidebar:
    ...
    # 🌟 檢查機制：如果使用者還沒輸入任何東西獲利指標組
    if not stock_code.strip():
        st.error("❌ 請先輸入台股代碼以開始智能分析！")
        st.stop()  # 🌟 關鍵防呆：直接在這裡暫停，不讓後面的財報模組因為空值而當機
    ...
    # 🌟 加上這兩行「偷天換日」翻譯官！
    # 甭管後面程式碼寫得有多刁鑽，只要把變數變回原本的文字，後面所有功能立刻全部認得！
