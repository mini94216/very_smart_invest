...
# 匯入你的認證模組
from streamlit_auth import register_user, verify_login, check_user_exists


# ==== 2. 初始化 Session State (一定要在存取任何 session_state 之前) ====
...
st.markdown(
    ...
    .dark-mode {
        ...
        --line-0: rgba(255, 255, 255, 0.10);
        --surface: rgba(45, 41, 51, 0.9);
    }
...
# ==== 全域常數 ====
from pathlib import Path
#DB_PATH = Path(__file__).parent / "streamlit.db"
...
def make_macd_figure_plotly(
    ...
    try:
        ...
        # ===== 用策略訊號畫圖（取代 trades_detail）=====
        buy_x = pd.to_datetime([t["entry_time"] for t in trades_detail]) if trades_detail else []
        sell_x = pd.to_datetime([t["exit_time"] for t in trades_detail]) if trades_detail else []
        ...
        for t in buy_x:
            if t in df2.index:
                buy_y.append(df2.loc[t, "Close"])

        for t in sell_x:
            if t in df2.index:
                sell_y.append(df2.loc[t, "Close"])
    ...
    if auth_mode == "會員登入":
        ...
        with st.form(key="login_form", clear_on_submit=False):
            ...
            if login_submit:
                ...
                result = verify_login(final_userid, final_password)
...
with st.sidebar:
    ...
    raw_mode = st.radio("分析模式", ["🐻個股財報分析", "🐻‍❄️MACD技術分析回測"], index=mode_index)
    
    if "個股財報分析" in raw_mode:
        selected_mode = "📄 模式一：個股財報分析（基本面）"
    else:
        selected_mode = "📈 模式二：技術分析回測（MACD）"

    st.markdown("---")
    st.link_button("🐶功能建議與回饋", "https://forms.gle/nDyRNa4BvSzuecqe8", use_container_width=True)
...
# 模組 2️⃣：營運效率組（全天數統一分析）
        ...
        with tab3:
            ...
            with col1:
                st.markdown("### 💐訂單真實性驗證")
                st.info("**白話導讀**：『合約負債-流動』就是客戶預付的訂金。若存貨增加、訂金也同步暴增，代表都在為大單備貨，不是產品滯銷！")
                
                # 精準對接 TEJ 欄位名稱
                inventory_col = get_column_by_keyword(df, ['存貨'])
                liability_col = get_column_by_keyword(df, ['合約負債-流動', '合約負債'])
                inv_days_col = get_column_by_keyword(df, ['存貨週轉天數'])
                
                if inventory_col is not None and liability_col is not None:
                    latest_inventory = float(df.iloc[-1, inventory_col])
                    latest_liability = float(df.iloc[-1, liability_col])
                    prev_inventory = float(df.iloc[-2, inventory_col]) if len(df) > 1 else latest_inventory
                    prev_liability = float(df.iloc[-2, liability_col]) if len(df) > 1 else latest_liability
                    ...
                    # 抓取現成的存貨週轉天數
                    latest_days_str = ""
                    if inv_days_col is not None:
                        latest_days_str = f"（目前貨物從進貨到賣掉平均需要 {float(df.iloc[-1, inv_days_col]):.1f} 天）"
                    
                    # 高級判決邏輯
                    if liability_growth > 0.5 * inv_growth and inv_growth > 0:
                        verdict = "真有大單 (珍珠) ✨"
                        color = "green"
                        detail = "存貨上升，但客戶給的訂金（合約負債）同步暴增！代表堆積的存貨全是為了應付後續大訂單，未來營收有保障。"
                    elif inv_growth > 0.2 and liability_growth < 0.05:
                        verdict = "有滯銷風險 (陷阱) ⚠️"
                        color = "red"
                        detail = f"倉庫存貨明顯變多，但買方訂金卻停滯不前。需要高度小心貨物賣不出去、塞滿倉庫的危機！{latest_days_str}"
                    else:
                        verdict = "供需平衡 (正常) 📊"
                        color = "blue"
                        detail = "存貨與買方訂金變動平穩，公司正按照正常的市場節奏與訂單量在備貨。"
                    ...
                    fig.add_trace(go.Scatter(
                        x=df['季度_label'],
                        y=df.iloc[:, inventory_col].tolist(),
                        mode='lines+markers',
                        name='存貨（公司備貨）',
                    ...
                    fig.add_trace(go.Scatter(
                        x=df['季度_label'],
                        y=df.iloc[:, liability_col].tolist(),
                        mode='lines+markers',
                        name='合約負債-流動（客戶訂金）',
                    ...
                    fig.update_layout(
                        title="📊 存貨 vs 客戶訂金趨勢對比",
                        hovermode="x unified",
                        xaxis_title="季度",
                        yaxis_title="存貨（千元）",
                        yaxis2=dict(title="合約負債-流動（千元）", overlaying='y', side='right', showgrid=False),
                        height=350,
            ...
            with col2:
                st.markdown("### 💐獲利含金量驗證")
                st.info("**白話導讀**：EPS 是會計師在紙上算的利潤，營運現金流則是口袋收到實實在在的現鈔。賺錢卻收不到現鈔的公司最危險！")
                
                eps_col = get_column_by_keyword(df, ['每股盈餘', 'EPS'])
                ocf_col = get_column_by_keyword(df, ['來自營運之現金流量', '來自營運', 'OCF'])
                
                if eps_col is not None and ocf_col is not None:
                    latest_eps = float(df.iloc[-1, eps_col])
                    latest_ocf = float(df.iloc[-1, ocf_col])
                    prev_eps = float(df.iloc[-2, eps_col]) if len(df) > 1 else latest_eps
                    prev_ocf = float(df.iloc[-2, ocf_col]) if len(df) > 1 else latest_ocf
                    
                    eps_trend = latest_eps >= prev_eps
                    ocf_trend = latest_ocf >= prev_ocf
                    
                    # 獲利純度交叉分流
                    if latest_eps > 0 and latest_ocf < 0:
                        verdict = "黑字倒閉風險 🚩"
                        color = "red"
                        detail = f"最新每股盈餘 ({latest_eps:.2f}元) 為正，但營運現金流卻是淨流出！代表利潤全卡在收不回來的欠款或賣不掉的倉庫裡。"
                    elif eps_trend and ocf_trend:
                        verdict = "高含金量 (真賺錢) ✅"
                        color = "green"
                        detail = f"帳面利潤與實收現金流同步成長！代表公司賺進來的錢都有紮實的現金做後盾，獲利純度極高。"
                    elif eps_trend and not ocf_trend:
                        verdict = "虛假獲利風險 ⚠️"
                        color = "orange"
                        detail = f"雖然帳面 EPS 創高，但實際進帳的現金流卻在下滑。需高度提防後續盈餘轉壞、應收帳款過高的問題。"
                    elif not eps_trend and ocf_trend and latest_ocf > 0:
                        verdict = "隱形吸金王 (極高) 💎"
                        color = "blue"
                        detail = "帳面 EPS 雖然下滑，但實際收到的現金卻逆勢大增！這通常是因為非現金的折舊費用較高，本業吸金能力其實一級棒。"
                    else:
                        verdict = "營運調整中 📉"
                        color = "gray"
                        detail = "每股盈餘與現金流量皆較上季衰退，顯示公司目前正處於營運低迷期或產業調整期。"
                    ...
                    fig.update_layout(
                        title="📊 EPS vs 營運現金流趨勢對比",
                        hovermode="x unified",
                        xaxis_title="季度",
                        yaxis_title="EPS（元）",
                        yaxis2=dict(title="營運現金流（千元）", overlaying='y', side='right', showgrid=False),
                        height=350,
                        margin=dict(l=10, r=10, t=60, b=10)
                    )
                    st.plotly_chart(fig, use_container_width=True)
            ...
            with col3:
                ...
                st.info("**白話導讀**：CCC (現金循環週期) 代表資金卡在外面收不回來的天數。**天數越短，收錢手腳越快、資金週轉效率越高！**")
                
                if 'CCC' in df.columns:
                    ccc_values = df['CCC'].dropna()
                    
                    if len(ccc_values) >= 2:
                        latest_ccc = float(ccc_values.iloc[-1])
                        slope = (latest_ccc - float(ccc_values.iloc[0])) / len(ccc_values)
                        
                        if slope < -5:
                            verdict = "營運大翻轉 (極佳) ✨"
                            color = "green"
                            detail = f"最新循環週期大幅縮短至 {latest_ccc:.1f} 天！代表公司催收帳款與賣貨的速度明顯加快，資金調度壓力大解放。"
                        elif slope < 0:
                            verdict = "體質逐步改善 ⬇️"
                            color = "blue"
                            detail = f"最新週期為 {latest_ccc:.1f} 天，呈現溫和緩步下降的健康趨勢，營運體質正默默優化。"
                        else:
                            verdict = "資金效率惡化 ⚠️"
                            color = "red"
                            detail = f"最新天數飆升至 {latest_ccc:.1f} 天！錢卡在外面收不回來的時間變長了，小心正面臨客戶壓款或滯銷風險。"
            ...
            with col4:
                st.markdown("### 💐低估值純度檢查")
                st.info("**白話導讀**：買股票大家都想買低本益比，但要提防公司是不是靠『賣大樓祖產』一次性暴富。我們必須搭配本業獲利能力（營業利益率）來拆穿它的謊言！")
                
                pe_col = get_column_by_keyword(df, ['當季季底P/E', 'P/E'])
                op_col = get_column_by_keyword(df, ['營業利益率'])
                
                if pe_col is not None:
                    latest_pe = float(df.iloc[-1, pe_col])
                    latest_op = float(df.iloc[-1, op_col]) if op_col is not None else 10.0
                    
                    if latest_pe >= 18:
                        verdict = "估值不夠便宜 ❌"
                        color = "red"
                        detail = f"目前當季季底 P/E 為 {latest_pe:.1f} 倍，價格處於相對高位，不具備好入手的安全邊際。"
                    else:
                        # 搭配本業營業利益率進行二次驗證（設定基本安全門檻為 8%）
                        if latest_op >= 8.0:
                            if latest_pe < 11:
                                verdict = "黃金純低估 (大推) 💎"
                                color = "green"
                                detail = f"本益比極低 ({latest_pe:.1f}倍)，且本業賺錢實力驚人（營業利益率高達 {latest_op:.1f}%）！這是真正的真金白銀低估標的。"
                            else:
                                verdict = "物美價廉 (相對純) ⭐"
                                color = "blue"
                                detail = f"本益比合理偏低 ({latest_pe:.1f}倍)，且本業營益率 ({latest_op:.1f}%) 穩定，屬於價格划算、體質健全的好公司。"
                        else:
                            verdict = "地雷價值陷阱 ⚠️"
                            color = "orange"
                            detail = f"雖然本益比只有 {latest_pe:.1f} 倍看似便宜，但本業獲利能力極差（營業利益率僅 {latest_op:.1f}%）。高度懷疑是靠一次性業外收入撐場的糖衣陷阱！"
                    ...
                    fig.update_layout(
                        title="📊 P/E 估值與本業獲利純度對比趨勢",
                        hovermode="x unified",
                        xaxis_title="季度",
                        yaxis_title="當季季底 P/E（倍）",
                        yaxis2=dict(title="營業利益率（%）", overlaying='y', side='right', showgrid=False) if op_col is not None else None,
                        height=350,
                        margin=dict(l=10, r=10, t=60, b=10)
                    )
                    st.plotly_chart(fig, use_container_width=True)
                ...
                try:
                    ...
                    if data_clean is None or data_clean.empty:
                        ...
                        if equity_df is None or stats_summary is None:
                            ...
                            # 🚀 原本的東西完全保留，只在對的時間點進行圖表微調
                            if fig is not None:
                                # 🎯 核心修正 1：遍歷圖內所有資料軌跡（K線、均線、買賣點）
                                # 將它們的 X 軸全數轉換成標準的年-月-日字串，點位就會一隻一隻精準黏回K線上！
                                for trace in fig.data:
                                    if trace.x is not None:
                                        try:
                                            trace.x = pd.to_datetime(trace.x).strftime('%Y-%m-%d')
                                        except Exception:
                                            pass

                                # 🎯 核心修正 2：設定 X 軸型態，並讓字體正立、不擁擠
                                fig.update_xaxes(
                                    type='category',     # 移除國定假日與週末空缺
                                    tickangle=0,         # ✨ 字體完全正立，不用斜體
                                    nticks=10            # ✨ 限制畫面上只出現大約 10 個乾淨日期，避免重疊
                                )
                            ...
                            if trades_detail:
                                ...
                                if '報酬率(%)' in trades_df.columns:
                                    trades_df['報酬率(%)'] = trades_df['報酬率(%)'].map(lambda x: f"{x:+.2f}%" if pd.notnull(x) else "")
                                if '交易損益' in trades_df.columns:
                                    trades_df['交易損益'] = trades_df['交易損益'].map(lambda x: f"${x:,.2f}" if pd.notnull(x) else "")
